...
export const fetchBySearch = async (params: string) => {
  const response = await fetch(
    `https://api.themoviedb.org/3/search/multi?query=${params}&api_key=${process.env.MOVIESDB_API_KEY}&language=en-US&page=1`
  );
  return response.json();
};
